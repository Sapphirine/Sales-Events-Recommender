...
class CreateIndex:
    ...
    def getTerms(self, line):
        ...
        line=line.lower()
        line=re.sub(r'[^a-z ]',' ',line) #put spaces instead of non-alphanumeric characters
        line=line.split()
    ...
    def parseCollection(self):
        ...
        curPage = doc
        pageid2 = [curPage[x] for x in range(len(curPage)) if x%3 == 0]
		#print pageid2
        pageid = [y[0:len(y)-1] for y in pageid2]
        pageInfo2 = [curPage[x] for x in range(len(curPage)) if x%3 == 1]
        pageInfo = [y[0:len(y)-1] for y in pageInfo2]
        pagetext = [curPage[x] for x in range(len(curPage)) if x%3 == 2]
		
        if pageid==None or pagetext==None:
            return {}

        d={}
        d['id']=pageid#.group(1)
        d['info']=pageInfo#.group(1)
    ...
    def createIndex(self):
        ...
        #while pagedict != {}:
        for m in range(len(pagedict['id'])):
            lines=pagedict['text'][m]
            pageid=int(pagedict['id'][m])
            terms=self.getTerms(lines)
            
            #self.titleIndex[pagedict['id']]=pagedict['id']
...
#14/12/08
#Use: python createIndex_amazonCat.py stopwords.dat total.txt catTFIDF.dat
