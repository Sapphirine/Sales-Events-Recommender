...
class QueryIndex:
    ...
    def rankDocuments(self, terms, docs):
        ...
        #print '\n'.join(str(resultDocs)), '\n'
        for x in range(len(resultDocs)):
            print(str(resultDocs[x]+1)+'  '+self.pageInfo[resultDocs[x]][:-5])
            #print(self.pageInfo[resultDocs[x]])
            #print self.pageInfo[resultDocs[x]],
            #print(bb.row_values(resultDocs[x]))
        print('================================')
    ...
    def readtext(self):
        f = open(self.article, 'r')
        tmp = []
        for doc in f:
            tmp.append(doc)
        f.close()
        self.pageInfo = [tmp[x] for x in range(len(tmp)) if x%3 == 1]
        self.pagetext = [tmp[x] for x in range(len(tmp)) if x%3 == 2]
        print(len(self.pagetext))
...
#14/12/8
#Use: python queryIndex_amazonCat.py stopwords.dat catTFIDF.dat total.txt